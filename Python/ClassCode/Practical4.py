...
sql = "INSERT INTO StudentMarks (RegNo, Name, Gender, CGPA, State, Country)\
VALUES (%s, %s, %s, %s, %s, %s)"
val = [(101, 'John Doe', 'Male', 8.5, 'California', 'USA'),
    ...
    (145, 'Anjali Rai', 'Female', 9.7, 'UP', 'India'),
    ...
    (150, 'Tristan Cane', 'Male', 10, 'New York', 'USA')
   ]  
c.executemany(sql, val)

c.execute("SELECT * FROM StudentMarks WHERE CGPA = (SELECT MIN(CGPA) FROM StudentMarks);")
print("Lowest CGPA:")
print(c.fetchone())

c.execute("SELECT * FROM StudentMarks WHERE CGPA = (SELECT MAX(CGPA) FROM StudentMarks);")
print("Highest CGPA:")
print(c.fetchone())

c.execute("SELECT * FROM StudentMarks ORDER BY CGPA DECS;")
print("Highest CGPA:")
print(c.fetchall())

c.execute("SELECT * FROM StudentMarks ORDER BY CGPA ASC;")
print("Lowest CGPA:")
print(c.fetchall())

c.execute('''SELECT * FROM StudentMarks
          WHERE CGPA = (SELECT MAX(CGPA) FROM StudentMarks WHERE Gender = 'Male');''')
print("Highest CGPA in Boys:")
print(c.fetchone())

c.execute('''SELECT * FROM StudentMarks 
          WHERE CGPA = (SELECT MAX(CGPA) FROM StudentMarks WHERE Gender = 'Female');''')
print("Highest CGPA in Girls:")
print(c.fetchone())

c.execute('''SELECT * FROM StudentMarks
          WHERE CGPA = (SELECT MIN(CGPA) FROM StudentMarks WHERE Gender = 'Male');''')
print("Highest CGPA in Boys:")
print(c.fetchone())

c.execute('''SELECT * FROM StudentMarks 
          WHERE CGPA = (SELECT MIN(CGPA) FROM StudentMarks WHERE Gender = 'Female');''')
print("Highest CGPA in Girls:")
print(c.fetchone())

c.execute(''' SELECT * FROM StudentMarks
          WHERE CGPA < 8 AND CGPA > 7;''')
print(c.fetchone())

c.execute('''UPDATE StudentMarks
          SET CGPA = 7.5
          WHERE CGPA < 7.5;''')
print("CGPA update Succesfully")

c.execute('''SELECT AVG(CGPA) FROM StudentMarks;''')
print("Averate CGPA:")

c.execute('''SELECT COUNT(DISTINCT State) AS NumberOfStates
          FROM StudentMarks;''')
print(c.fetchall())
c.execute('''SELECT COUNT(DISTINCT Country) AS NumberOfCounties
          FROM StudentMarks;''')
print(c.fetchall())

c.execute('''SELECT COUNT(*) AS NumberOfNRIStudents 
FROM StudentMarks
WHERE Country <> 'India';
''')
print(c.fetchall())
